...
`_extract_token_payload_from_manager` decides which error the client sees, and that
decision is also what keeps the refresh path reachable when more than one domain is
configured. Those are two different jobs, and they want opposite answers, which is why the
method takes a keyword-only `allow_refresh`.

With several domains configured, one call can end up holding both an `UnknownKeyIdError`
from a domain that lacks the `kid` and some other error from a domain that got further.

- On the **first pass** (`allow_refresh=True`, from `__call__`) the `UnknownKeyIdError`
  wins. That pass exists only to decide whether a refresh is worth a thread hop, and an
  unknown `kid` is the one failure a refresh can repair. Demoting it here would leave a
  genuine key rotation unrecovered whenever another domain happens to be misconfigured,
  which is an outage months later traded for a tidier status code today.
- On the **retry pass** (`allow_refresh=False`, from `_refresh_and_retry`) the refresh has
  already been tried and did not help, so `last_error` wins. A `PayloadMappingError` from a
  bad `permission_extractor` then reaches the client as the 500 it is, rather than as a 401
  that sends whoever is debugging it after the wrong problem.
- With one configured domain the flag changes nothing, because there is only ever one error.

Two further rules hold on both passes. Any non-`UnknownKeyIdError` decode failure is
remembered as the last real failure rather than discarded, so its own status and detail
survive instead of being flattened into a generic "no matching JWK". And a `match_keys`
refusal deliberately bypasses all of this: `_check_match_keys` runs outside the loop's
`try`, after a manager has successfully decoded the token, so its `AuthorizationError`
(403) propagates immediately instead of being demoted to a 401 by the fallthrough. The
token verified; it simply is not for this caller, and those are different answers.
